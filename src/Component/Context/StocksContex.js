...
const StocksContext = ({ children }) => {
  ...
  const getAllHistorySalesStockData = async (loginuserid, clientdata) => {
    let allHistorySalesStockData =
      localstorage.addOrGetAllHistorySalesStockData("", "get");
    if (loginuserid != "" || loginuserid != null) setloginuser(loginuserid);
    let getSalesStockfromdb = await stockDb.getAllHistorySalesStockDB(
      loginuserid
    );
    ...
    if (getSalesStockfromdb.status === 200) {
      ...
      let deriveClientDetailValue = deriveClientDetail(
        getSalesStockfromdb.data,
        clientdata,
        "sale"
      );
      localstorage.addOrGetAllHistorySalesStockData(
